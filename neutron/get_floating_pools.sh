...
echo "router_id;external_network_id;external_network_name;internal_network_id" >> $output
...
for router in $(neutron router-list | grep enable_snat | awk '{ print $2 }'); do
    ...
    external_name=$(neutron net-show $external | grep name | awk -F\| '{ print $3 }' | tr -d '[[:space:]]')
    for subnet in $(neutron router-port-list $router | grep subnet | awk -F\" '{ print $4 }'); do
        ...
        if [[ $network != $external ]]; then
            echo "$router;$external;$external_name;$network" >> $output
        fi
    done
done
